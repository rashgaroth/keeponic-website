...
import { BAD_WORD } from '../../../constants/badWord';
...
function beforeUpload(file) {
	...
	const isLt2M = file.size < 5242880;
	if (!isLt2M) {
		message.error('Gambar tidak boleh melebihi 5 MB');
	}
...
class ProductAdd extends Component {
	constructor(props) {
		...
		this.formTest = createRef();
		this.formTest2 = createRef();
	}
	...
	handleChangeFirst = async (info) => {
		...
		getBase64(info.file.originFileObj, () => {
			this.setState({
				imageFirst: compressImg[0].prefix + compressImg[0].data,
				loadingFirst: false,
	...
	handleChangeSecond = async (info) => {
		...
		getBase64(info.file.originFileObj, () =>
			this.setState({
				imageSecond: compressImg[0].prefix + compressImg[0].data,
				loadingSecond: false,
	...
	handleChangeThird = async (info) => {
		...
		getBase64(info.file.originFileObj, (imageUrl) =>
			this.setState({
				imageThird: compressImg[0].prefix + compressImg[0].data,
				loadingThird: false,
	...
	handleChangeFourth = async (info) => {
		...
		getBase64(info.file.originFileObj, (imageUrl) =>
			this.setState({
				imageFourth: compressImg[0].prefix + compressImg[0].data,
				loadingFourth: false,
	...
	handleChangeName = (e) => {
		this.setState({
			...
			lengthName: e.target.value.length,
		});
	};
	...
	checkWeight = (_, value) => {
		if (value.number < 100 || value.number > 1000000) {
			return Promise.reject(
				new Error(
					'Berat produk yang diperbolehkan antara 100 sampai 1000000 gram'
				)
			);
		} else {
			this.setState({
	...
	checkProfanityWord = (_, value) => {
		if (value !== 0) {
			const foundSwears = BAD_WORD.filter((word) => value.includes(word));
			if (foundSwears.length) {
				return Promise.reject(
					new Error(
						'Deskripsi mengandung kata kasar, periksa dan perbaiki kembali.'
					)
				);
			}
		}
	...
	submitAllForm = async () => {
		...
		if (imageFirst === '') {
			...
			data = {
				...
				// category: formInformation.category,
				description: formInformation.description,
				// condition: formInformation.condition,
				price: formInformation.price.number,
				stock: formInformation.stock.number,
				// weight: formDelivery.weight,
				// length: formDelivery.length,
				// width: formDelivery.width,
				// height: formDelivery.height,
				status: 1,
			};
			data ? this.props.addProduct(data) : console.log('Failed');
		}
	};
	...
	render() {
		...
		console.log(Math.round(volumetrikPrice));
								...
								<Col lg={12}>
									...
									>
										<Form
											...
											initialValues={{
												category: this.state.categoryProduct.toString(),
										...
										>
											...
											<Form.Item
												...
												rules={[
													...
													// {
													// 	validator: this.checkProfanityWord,
													// },
												]}
											>
												<Row>
...
const mapStateToProps = (state) => {
	const { loading } = state.ProductReducers;
