		...
		window.onscroll = () => {
			switch (window.innerHeight) {
				...
				case 823:
					...
					break;
				default:
					setTargetOffset(window.scrollY < window.innerHeight - 20);
					break;
			}
		};
	}, []);
...
const ButtonSubmitDraft = ({ onClick, loading, onClickDraft }) => {
	const [targetOffset, setTargetOffset] = useState(true);
...
class ProductAdd extends Component {
	...
	componentDidMount() {
		...
		onChangeStatePathInfo('path', this.props.match.params.slug);
		getProductDetail(this.props.match.params.id);
		getProductAllCategories();
	}

	componentWillReceiveProps(nextProps) {
		if (nextProps.detailProduct !== this.props.detailProduct) {
			this.setState({
				nameProduct: nextProps.detailProduct.nameProduct,
				categoryProduct:
					nextProps.detailProduct.categoryProduct !== null
						? nextProps.detailProduct.categoryProduct.id
						: 1,
				descProduct: nextProps.detailProduct.descProduct,
				priceProduct: nextProps.detailProduct.priceProduct,
				stockProduct: nextProps.detailProduct.stockProduct,
				imageFirst: nextProps.detailProduct.imageFirst,
				imageSecond: nextProps.detailProduct.imageSecond,
				imageThird: nextProps.detailProduct.imageThird,
				imageFourth: nextProps.detailProduct.imageFourth,
				status: nextProps.detailProduct.status,
				weightProduct: nextProps.detailProduct.weightProduct,
				lengthProduct: nextProps.detailProduct.lengthProduct,
				widthProduct: nextProps.detailProduct.widthProduct,
				heightProduct: nextProps.detailProduct.heightProduct,
				createdDate: nextProps.detailProduct.createdDate,
				weight: nextProps.detailProduct.weightProduct,
			});
		}
	}
	...
	checkWeight = (_, value) => {
		...
		if (value.number < 1 || value.number > 1000000) {
			return Promise.reject(
				new Error(
					'Berat produk yang diperbolehkan antara 1 sampai 1000000 gram'
				)
			);
		} else {
			this.setState({
	...
	checkWidth = (_, value) => {
		if (value.number < 0 || value.number > 30000) {
			return Promise.reject(new Error('Hanya antara 1 sampai 30000'));
		} else {
			this.setState({
	...
	submitAllForm = async () => {
		...
		if (imageFirst === '') {
			...
			if (formInformation && formDelivery !== {}) {
				let data = {};
				detailProduct.imageFirst !== this.state.imageFirst &&
					(data['avatar'] = this.state.imageFirst);

				detailProduct.imageSecond !== this.state.imageSecond &&
					(data['second_avatar'] = this.state.imageSecond);

				detailProduct.imageThird !== this.state.imageThird &&
					(data['third_avatar'] = this.state.imageThird);

				detailProduct.imageFourth !== this.state.imageFourth &&
					(data['fourth_avatar'] = this.state.imageFourth);

				data['name'] = formInformation.name.trim();
				data['t_category_id'] = formInformation.category;
				data['description'] = formInformation.description.trim();
				data['price'] = formInformation.price.number;
				data['stock'] = formInformation.stock.number;
				data['weight'] = formDelivery.weight.number;
				data['lengths'] = formDelivery.length.number;
				data['width'] = formDelivery.width.number;
				data['height'] = formDelivery.height.number;
				data['status'] = 1;
				// data['is_new'] = formInformation.condition;
	...
	submitAllFormUpdateToDraft = async () => {
		const { detailProduct } = this.props;
		const { imageFirst } = this.state;
		if (imageFirst === '') {
			message.error({
	...
	submitAllFormUpdateToShow = async (next) => {
		const { detailProduct } = this.props;
		const { imageFirst } = this.state;
		if (imageFirst === '') {
			message.error({
							...
							<Col lg={10} md={9}>
								{!status ? (
									<Affix offsetTop={70}>
										<Alert
											className='mb-2'
											message='Produk ini diarsipkan oleh Kamu'
											type='warning'
											showIcon
										/>
									</Affix>
								...
								<Row id='uploadphoto'>
									<Col lg={12}>
										<Card title='Foto Produk' bordered={false}>
											<Card style={{ border: 'dashed 2px #BEBEBE' }}>
															...
															<Row className='mb-2'>
																<Col xs={5} lg={5}>
																	<Button
																		...
																		style={{ fontSize: 11 }}
																		onClick={() =>
																			this.handlePreview(imageSecond, 'Samping')
																		}
																	>
																		Lihat
																	</Button>
															...
															<Row className='mb-2'>
																<Col xs={5} lg={5}>
																	<Button
																		...
																		style={{ fontSize: 11 }}
																		onClick={() =>
																			this.handlePreview(imageThird, 'Belakang')
																		}
																	>
																		Lihat
																	</Button>
															...
															<Row className='mb-2'>
																<Col xs={5} lg={5}>
																	<Button
																		...
																		style={{ fontSize: 11 }}
																		onClick={() =>
																			this.handlePreview(imageFourth, 'Lainnya')
																		}
																	>
																		Lihat
																	</Button>
									...
									<Col lg={12}>
										...
										>
											<Form
												...
												initialValues={{
													name: nameProduct,
													category: categoryProduct.toString(),
									...
									<Col lg={12}>
										...
										>
											<Form
												...
												initialValues={{
													weight: {
														number: weightProduct,
													},
													length: {
														number: lengthProduct,
													},
													width: {
														number: widthProduct,
													},
													height: {
														number: heightProduct,
													},
												}}
												autoComplete='off'
											>
								...
								<Affix offsetBottom={0}>
									{!status ? (
										<ButtonSubmitDraft
											onClick={this.submitAllFormDraft}
											onClickDraft={this.submitAllFormUpdateToShow}
											loading={loading}
										/>
									) : (
										<ButtonSubmit
											onClick={this.submitAllForm}
											onClickDraft={this.submitAllFormUpdateToDraft}
											loading={loading}
										/>
									)}
								</Affix>
...
const mapStateToProps = (state) => {
	const { loading, detailProduct, categories, error } = state.ProductReducers;
